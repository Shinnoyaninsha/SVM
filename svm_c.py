...
class SVM_C:
    def __init__(self,kernel=None, learning_rate=0.01, c_param=1, n_iters=1000, degree=3, bias=1, gamma=1):
        self.w = None
        self.b = None
    ...
    def fit(self, X, y):
        ...
        if self.kernel is None: #si la SVM n'utilise pas de kernel, on utilise la descente de gradient pour actualiser le vecteur de poids w
            ...
            for _ in range(self.n_iters):
                for i in range(n_samples):
                    ...
                    for j in self.support_vectors:
                        ...
                        prediction += self.alpha[j[0]]* self.y_train[j[0]] * self._kernel(self.X_train[i], self.X_train[j[0]])
                ...
                self.support_vectors = [(i, a) for i, a in enumerate(self.alpha) if 0 < a]
            ...
            #         self.alpha[j[0]] * self.y_train[j[0]] * self._kernel(self.X_train[idx], self.X_train[j[0]])
            #         for j in self.support_vectors
            #     )

            # if self.support_vectors:
            #     # Compute the average bias using all the support vectors
            #     total_bias = 0
            #     for (idx, _) in self.support_vectors:
            #         bias_i = self.y_train[idx] - sum(
            #             self.alpha[j[0]] * self.y_train[j[0]] * self._kernel(self.X_train[idx], self.X_train[j[0]])
            #             for j in self.support_vectors
            #         )
            #         total_bias += bias_i
            ...
            #     self.b = total_bias / len(self.support_vectors)
    ...
    def predict(self, X):
        ...
        if self.kernel is None:
            ...
            for x in X:
                ...
                #     prediction += self.alpha[j] * self.y_train[j] * self._kernel(x, self.X_train[j])
                for j in self.support_vectors:
                    prediction += self.alpha[j[0]]* self.y_train[j[0]] * self._kernel(x, self.X_train[j[0]])
